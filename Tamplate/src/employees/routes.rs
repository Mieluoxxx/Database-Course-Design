...
async fn find_all()->Result<HttpResponse,CustomError>{
    let employees = web::block(||Employees::find_all()).awit.unwrap()?;
...
#[delete("/employees/{id}")]
async fn remove(id: web::Path<i32>) -> Result<HttpResponse,CustomError>{
    let res = Employees::delete(id.into_inner())?;
